...
double func(double x) {
	return (double)a * x + b * sin(c * x * M_PI);
}
...
int main() {
	...
	double s = 0.00000000000, t = 1000.00000000000;
	double ans = s;
	while (1) {
		double mid = (s + t) / 2.00000;
		if (abs(func(mid) - 100.0000000) < eps) {
			ans = mid; break;
		}
		else {
			if (func(s) > 100.000000) {
				s = mid;
			}
			else if (func(t) < 100.000000) {
				t = mid;
			}
			else {
				if (func(mid) > 100.000000)t = mid;
				else s = mid;
			}
		}
	}
